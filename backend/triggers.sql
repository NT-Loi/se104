...
BEFORE UPDATE ON api_CT_HoaDon
FOR EACH ROW
BEGIN
    SELECT CASE 
        WHEN (SELECT SoTienNo FROM api_KhachHang WHERE MaKhachHang = (SELECT MaKH_id FROM api_HoaDon WHERE MaHD = NEW.MaHD_id)) > (SELECT NoTD FROM api_thamso WHERE id = 1)
...
-- Trigger tính thành tiền khi thêm hóa đơn
...
-- Trigger tính thành tiền khi cập nhật hóa đơn
...
BEGIN
    ...
    SET NoDau = COALESCE(
        (SELECT NoCuoi 
         ...
         LIMIT 1),
        (SELECT SoTienNo FROM api_KhachHang WHERE MaKhachHang = NEW.MaKH_id)
    ),
